...
const chooseFiles = async function(file) {
  ...
  JSON.parse(filesAll).tree.forEach(item => {
    if (item.path === fileContent[0].path && item.type === 'blob') {
      const endFilePath = item.path.substring(item.path.lastIndexOf('\/') + 1, item.path.length)
      const preFilesPath = item.path.substring(0, item.path.lastIndexOf('\/') + 1)

      log(`写入地址 ${path.resolve(file, '../' + preFilesPath)}`)
      log(mkdirsSync(path.resolve(file, '../' + item.path)))
      // let endUrl = item.path.substring(item.path.lastIndexOf('\/') + 1, item.path.length)
      fs.writeFileSync(path.resolve(file, '../' + preFilesPath), Buffer.from(item.content, 'base64').toString('utf8'))
    }
  })
  // fs.writeFileSync(file, Buffer.from(fileContent.content, 'base64').toString('utf8'))
}
